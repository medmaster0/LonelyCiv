...
vector<Tower> build_neighborhood(vector<vector<Item>>* map_scenery_top,vector<vector<Item>>* map_scenery_bottom, bool* block_map, int map_width, int map_height, SDL_Color brick_p_col_in, SDL_Color brick_s_col_in, SDL_Color floor_p_col_in, SDL_Color floor_s_col_in,  SDL_Color door_col1, SDL_Color ladder_col_p ){
    ...
    int address_number; //a variable used to keep track of address numbering
    ...
    address_number = rand()%20;
    if(address_number%2 == 0){ address_number = address_number + 1; } //make sure these top ones are odd
    ...
    while(x1<map_width-35-25){
        ...
        build_x = x1 + build_dim + 3; //use prev build coords and leave space for walkways
        build_dim = rand()%4 + 5;
        ...
        //build a mailbox.
        temp_tower.build_mailbox(map_scenery_top, block_map, map_width, map_height, 1, to_string(address_number) + " " + street_name ); //position 1
        ...
        address_number = address_number + rand()%9 + 1;
        if(address_number%2 == 0){ address_number = address_number + 1; } //make sure these top ones are odd
...
//    //Build *OTHER* FIRST street (Top-Running Row) INSIDE, OPPOSITE
//    //==========================w==============
//    x1 = 0;
//    //y1 stays the same from last time
//    //y1 = 35 + rand()%5 - rand()%5;
//    x2 = map_width-1;
//    y2 = y1;
//
//    //no need to build another pair of signposts or street
//
//    //Initialize address stuff
//    address_number = rand()%20;
//    if(address_number%2 == 1){ address_number = address_number + 1; } //make sure these top ones, opposite are even
//
//    //Place towers all along the path...
//    x1 = 35;//move the x cursor on the other side of the vertical column path
//    while(x1<map_width-35-25){
...
//
//        //Now build a tower there;
//        Tower temp_tower_2 = Tower(map_scenery_top, map_scenery_bottom, block_map, map_width, map_height, build_x, build_y, build_dim, build_floors);
//
//        //Need to create a door in the correct place...
//        temp_tower_2.build_door(map_scenery_top, block_map, map_width, map_height, 0, 0); //build door in position 1 (top of square)
//
//        //build a mailbox.
//        temp_tower_2.build_mailbox(map_scenery_top, block_map, map_width, map_height, 0, to_string(address_number) + " " + street_name); //position 0
//
//        //put the backyard
//        temp_tower_2.assign_backyard(0);
//
//        //now connect tower to road by path
//        //first point is the step in front of door
//        x2 = temp_tower_2.door_x;
//        y2 = temp_tower_2.door_y + 1;
//        //Find path between two buildings
//        vector<vector<int>> path = A_Star(block_map, map_width, map_height, x1, y1, x2, y2); //a list of steps between the point and the street ref point
//        build_floor_path(map_scenery_bottom, block_map, map_width, map_height, x1, y1, x2, y2, floor_p_col_in, floor_s_col_in);
//
...
//
//        //add tower to running list
//        map_towers.push_back(temp_tower_2);
//
//    } //end other row
//
//
//
    ...
    //Initialize address stuff
    address_number = rand()%20;
    if(address_number%2 == 1){ address_number = address_number + 1; } //make sure these bottom ones are even
    ...
    while(x1<map_width-35-25){
        ...
        //build a mailbox.
        temp_tower.build_mailbox(map_scenery_top, block_map, map_width, map_height, 0, to_string(address_number) + " " + street_name); //position 0
    ...
    //Initialize address stuff
    address_number = rand()%20;
    if(address_number%2 == 1){ address_number = address_number + 1; } //make sure these right ones are even
    ...
    while(y1<map_height-35-15){
        ...
        //Initialize the build dimensions along the pathh....
        build_y = y1 + build_dim + 3 + 1; //use prev build_dim and leave space for balconies
        build_dim = rand()%4 + 5;
        ...
        //build a mailbox.
        temp_tower.build_mailbox(map_scenery_top, block_map, map_width, map_height, 2, to_string(address_number) + " " + street_name); //position 2
    ...
    //Initialize address stuff
    address_number = rand()%20;
    if(address_number%2 == 0){ address_number = address_number + 1; } //make sure these left ones are even
    ...
    while(y1<map_height-35-15){
        ...
        //build a mailbox.
        temp_tower.build_mailbox(map_scenery_top, block_map, map_width, map_height, 3, to_string(address_number) + " " + street_name); //position 3
