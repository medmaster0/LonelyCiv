...
void init_items(){
    ...
    for(int i = 0; i < 1500; i++){
        tempx = rand()%map_width-1;
        tempy = rand()%map_height-1;
        temp_tile = rand()%199;
        //Item temp_item = Item(tempx, tempy, temp_tile, world_colors[temp_tile],{255,255,255,0} ); //temporary item
        Item temp_item = Item(tempx, tempy, temp_tile, {static_cast<Uint8>(colorz[temp_tile][0]), static_cast<Uint8>(colorz[temp_tile][1]), static_cast<Uint8>(colorz[temp_tile][2])},{255,255,255,0} ); //temporary item
